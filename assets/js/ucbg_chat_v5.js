...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  let isOpen = false;
  let unreadCount = getIntCookie("chatUnread", 0); // panel kapalıyken birikiyor
  let lastSeenCount = getIntCookie("chatLastSeenCount", 0); // panel açıksa snapshot.size ile güncellenir
  ...
  nickSaveButton.addEventListener("click", () => {
    ...
    // Kullanıcı adı oluştuğunda unread'ı sıfırla
    unreadCount = 0;
    setIntCookie("chatUnread", 0);
    showBadge();
    updateBlurState();
  ...
  const baseQuery = query(messagesRef, orderBy("timestamp", "asc"), limitToLast(PAGE_SIZE)); // Son 50’yi getirir ve chat için idealdir [web:30]
  ...
  const unsubscribe = onSnapshot(baseQuery, { includeMetadataChanges: false }, (snapshot) => {
    ...
    if (isInitialLoad) {
      ...
      // Panel açıksa görüleni eşitle
      if (isOpen) {
        lastSeenCount = snapshot.size;
        setIntCookie("chatLastSeenCount", lastSeenCount);
      }
      return;
    }
    ...
    snapshot.docChanges().forEach((change) => {
      ...
      if (change.type === "added") {
        ...
        // Panel kapalıyken ve başkası yazdıysa unread artır
    ...
    // Panel kapalıyken addedCount kadar unread artır ve cookie’ye yaz
    if (!isOpen && addedCount > 0) {
      unreadCount += addedCount;
      setIntCookie("chatUnread", unreadCount);
      showBadge();
    }

    // Panel açıksa lastSeenCount = snapshot.size
    if (isOpen) {
      lastSeenCount = snapshot.size;
      setIntCookie("chatLastSeenCount", lastSeenCount);
    }

    // En alttaysa otomatik scroll
    if (userWasAtBottom && addedCount > 0) {
      scrollToBottom(false);
    }

    lastSnapshotSize = snapshot.size;
  }); // Realtime sadece son 50’lik pencere için tutulur [web:19][web:30]
  ...
  messagesDiv.addEventListener("scroll", async () => {
    if (messagesDiv.scrollTop <= 20 && !loadingOlder && oldestVisibleDoc) {
      ...
      try {
        const olderQuery = query(messagesRef, orderBy("timestamp", "asc"), endBefore(oldestVisibleDoc), limitToLast(PAGE_SIZE)); // endBefore + limitToLast yukarı yönlü sayfalama için kararlı desendir [web:41]
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      msgInput.value = "";
      // Gönderince daima en alta kaydır
      scrollToBottom(true);
    } catch (error) {
